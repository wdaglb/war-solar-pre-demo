...
import {Entity} from "solar/lib/ecs/Ecs";
import UnitUtil from "../../util/unit/UnitUtil";
import DataBase from "../../common/DataBase";
import TextTagUtil from "../../util/text/TextTagUtil";
import {TextTag} from "./texttag";
import BaseUtil from "../../util/BaseUtil";

let unitEntityMap: { [id: number]: Entity } = {}
...
export class Unit extends Widget {
    ...
    public textTag(text: string, red: number = 0, green: number = 200, blue: number = 0): TextTag {
        return TextTagUtil.textOnUnit(this.handle, text, red, green, blue)
    }
    ...
    public set entity(value: Entity) {
        unitEntityMap[this.id] = value
    }

    public get entity(): Entity {
        return unitEntityMap[this.id];
    }
    ...
    public hasItemType(itemType: number): boolean {
        for (let i = 0; i < 6; i++) {
            ...
            if (indexItem != null && GetItemTypeId(indexItem) == itemType) {
                return true
            }
        }
        return false
    ...
    public isAlive(): boolean {
        ...
        return this.getState(UNIT_STATE_LIFE) >= 1;
    }
